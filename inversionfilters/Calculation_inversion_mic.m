...
function [max_error_mic, v_irsens_df, v_h, v_entz]  = Calculation_inversion_mic(N, D, srate, f_min, f_max)

% Load impuls responses
c_files = dir('impulse_responses/Sens*');

v_irsens = [];
v_irsens_df = [];
for f = 1:length(c_files)
   s_tmp = load(['impulse_responses/' c_files(f).name]);
